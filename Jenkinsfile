pipeline {
    ...
    environment {
        ...
        DOCKERHUB_IMAGE = 'ikoyski/webtools-pse-data:latest'        
        DEPLOYMENT_FILENAME = 'Deploy-webtools-pse-data-private.yaml'
    }
    stages {
        stage('Git Stuff') {
            steps {
                script {
                	checkout scmGit(
                		...
                		extensions: [], 
                		userRemoteConfigs: [[url: 'https://github.com/ikoyski/webtools-pse-data.git']]
                	)
                }
            }
        }
